/*
	...
	*/
	...
	static int arm_execute_instruction(arm_core p) {
		uint32_t opcode, operand1, operand2, operand3 /*operands_result*/, rd, instr, bit25, bit23_24, bits26_27, bits4_11, res, arm_decode;     uint64_t operands_result;
					...
					if (ConditionPassed(p, cond_bits)) {
						...
						switch(opcode) {
							...
							case 0b0011: // RSB ; Reverse Subtract ; Rd := shifter_operand - Rn
								...
								break;
            /***********************************************************************************************************
                                   /*op_result devient 1 64 bits dont on teste les bits 32_63 pour a jour ls flags correspondant
                                      msj ds flags si S (bit 20) de cpsr vaut 1*/
...
/*#######la je pense q c plutot get_bit(flag,C)	*/		operand3 = get_bit(res, C);
...
void update_flag(arm_core p, uint64_t operands_result){
   ...
   if(get_bit(flag,20)==1){ //si S==1 
         ...
         if(get_bits(operands_result,63,32)!=0b0) //dans ce cas y'a retenue et overflow
             update_Flag_V(p,1);update_Flag_C(p,1);
    }
}
